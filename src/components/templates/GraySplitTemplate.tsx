...
const GraySplitTemplate: React.FC<GraySplitTemplateProps> = ({
  ...
  // 固定：左栏主要放基础信息 + 技能，其他走右栏
  const leftIds = new Set(["basic", "skills"]);
  const leftSections = sortedSections.filter((s) => leftIds.has(s.id));
  const rightSections = sortedSections.filter((s) => !leftIds.has(s.id));
  ...
  return (
    ...
    >
      {/* 灰色左栏 */}
      <aside
        className="h-full px-6 py-8 space-y-6"
      ...
      {/* 白色右栏 */}
      <main className="px-8 py-8 space-y-6 relative">
        {/* 中间竖线 */}
        <div
